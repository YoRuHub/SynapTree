...
import { setupUI, showContextMenu, hideContextMenu, hideSearch, searchNodes } from './ui.js';
...
try {
    State.Graph = ForceGraph3D()(elem)
        ...
        .linkColor(link => State.highlightLinks.has(link) ? 'rgba(255, 255, 0, 0.8)' : 'rgba(255, 255, 255, 0.25)')
        .linkWidth(1.5)
        .linkOpacity(1.0)
        .linkCurvature(0.2)
        ...
        .linkDirectionalParticleColor(link => State.highlightLinks.has(link) ? 'rgba(255, 255, 0, 0.6)' : '#ffffff');
...
function applyNodeStatus(targetId, newStatus) {
    ...
    // Try exact match
    let targetObj = State.pulseObjects.find(obj => obj.nodeId === targetId);

    // Try fuzzy
    if (!targetObj) {
        const lowerTarget = targetId.toLowerCase();
        targetObj = State.pulseObjects.find(obj => obj.nodeId.toLowerCase() === lowerTarget);
        if (targetObj) {
...
window.addEventListener('message', event => {
    ...
    if (message.command === 'setData') {
            ...
            if (message.data && message.data.nodes) {
                ...
                setTimeout(() => {
                    ...
                    if (State.isFirstLoad) {
                        State.Graph.cameraPosition({ z: 400 }, null, 1200);
                    ...
                    if (message.focusTargetId) {
                        ...
                        const targetNode = nodes.find(n => n.id === message.focusTargetId);
                        if (targetNode) {
                            const distance = 120;
                            const distRatio = 1 + distance / Math.hypot(targetNode.x, targetNode.y, targetNode.z);
                            const newPos = { x: targetNode.x * distRatio, y: targetNode.y * distRatio, z: targetNode.z * distRatio };
                    ...
                    if (State.pendingUpdates.size > 0) {
                        log(`Processing ${State.pendingUpdates.size} queued git updates...`);
        ...
        if (State.Graph) {
            const { nodes } = State.Graph.graphData();
            const targetNode = nodes.find(n => n.id === targetId || n.path === targetId);
            if (targetNode) {
                const distance = 120;
                const distRatio = 1 + distance / Math.hypot(targetNode.x, targetNode.y, targetNode.z);
                const newPos = { x: targetNode.x * distRatio, y: targetNode.y * distRatio, z: targetNode.z * distRatio };
